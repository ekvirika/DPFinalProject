...
def test_get_all_campaigns(
    ...
    mock_cursor = mock_db.get_connection.return_value.__enter__.return_value.cursor.return_value
    mock_cursor.fetchall.return_value = [
        {"id": str(campaign_id_1), "name": "Campaign 1", "campaign_type": CampaignType.DISCOUNT.value, "is_active": 1},
        {"id": str(campaign_id_2), "name": "Campaign 2", "campaign_type": CampaignType.BUY_N_GET_N.value,
         "is_active": 1}
    ]
...
def test_get_active_campaigns(
    ...
    # Check DB calls
    mock_cursor.execute.assert_called_once_with("SELECT * FROM campaigns WHERE is_active = 1")