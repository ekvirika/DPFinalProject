...
from core.models.receipt import ItemSold, RevenueByCurrency, Currency, ReceiptStatus, PaymentStatus
...
class SQLiteReportRepository:
    ...
    def generate_shift_report(self, shift_id: UUID) -> Optional[ShiftReport]:
        receipts = self.receipt_repository.get_by_shift(shift_id)
    ...
    def generate_sales_report(self) -> SalesReport:
        with self.db.get_connection() as conn:
            ...
            """, (PaymentStatus.COMPLETED.value,))
            result = cursor.fetchone()
            total_revenue_gel = result["total_gel"] if result and result["total_gel"] else 0
