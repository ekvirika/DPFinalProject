...
class CampaignService:
    ...
    def _validate_rules(self, campaign_type: str, rules: dict) -> None:
        if campaign_type == CampaignType.BUY_N_GET_N.value:
            ...
            if not all(field in rules for field in required_fields):
                raise CampaignValidationError(
                    f"Buy N Get N campaign requires fields: {', '.join(required_fields)}"
                )
        elif campaign_type == CampaignType.DISCOUNT.value:
            if "discount_percentage" not in rules:
                raise CampaignValidationError("Discount campaign requires discount_percentage")
            if not 0 <= rules["discount_percentage"] <= 100:
                raise CampaignValidationError("Discount percentage must be between 0 and 100")
